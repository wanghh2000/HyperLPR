...
def verticalEdgeDetection(image):
    ...
    flag, thres = cv2.threshold(
        image_sobel, 0, 255, cv2.THRESH_OTSU | cv2.THRESH_BINARY)
    print(flag)
    flag, thres = cv2.threshold(
        image_sobel, int(flag*0.7), 255, cv2.THRESH_BINARY)
...
def drawRectBox(image, rect, addText):
    ...
    cv2.rectangle(image, (int(rect[0]-1), int(rect[1])-16),
                  (int(rect[0] + 180), int(rect[1])), (0, 0, 255), -1, cv2.LINE_AA)
...
def SimpleRecognizePlateByE2E(image):
    ...
    for j, plate in enumerate(images):
        ...
        print(rect)
        if confidence > 0.7:
            image = drawRectBox(image, rect, res+" 置信度:" +
    ...
    for j, plate in enumerate(images):
        ...
        if len(val) == 3:
            ...
            if confidence/7 > 0.7:
                showTxt = res
                showTxt += " 可信度"
                showTxt += str(round(confidence * 100/7, 1)) + "%"
                image = drawRectBox(image, rect, showTxt)
                res_set.append(res)
